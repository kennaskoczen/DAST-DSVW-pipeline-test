...
LISTEN_ADDRESS, LISTEN_PORT = "13.59.230.200", 65412
...
def init():
    ...
    logging.basicConfig(
        ...
        level=logging.INFO,
        format='%(asctime)s [%(levelname)s] %(remote_addr)s requested %(path)s - %(message)s'
    )
    extra_info = {
        'remote_addr': request.remote_addr,
        'path': request.path
    }
    logging.info('Incoming request', extra=extra_info)
