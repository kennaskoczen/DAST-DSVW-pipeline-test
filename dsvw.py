...
def init():
    ...
    logging.basicConfig(
        filename='/var/log/dast-test/dast-test.log',
        level=logging.INFO,
    )
...
class ReqHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            logging.info({"DOCUMENT_ROOT": os.getcwd(), "HTTP_USER_AGENT": self.headers.get("User-Agent"), "REMOTE_ADDR": self.client_address[0], "REMOTE_PORT": self.client_address[1], "PATH": path, "QUERY_STRING": query})
            if path == '/':
                if "id" in params:
                    ...
                    content = subprocess.run("nslookup " + params["domain"], shell=True, env=os.environ, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE, text=True).stdout
                elif "xml" in params:
